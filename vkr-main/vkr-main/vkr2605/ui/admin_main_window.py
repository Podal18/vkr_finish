...
class Ui_AdminMainWindow(object):
    def setupUi(self, AdminMainWindow, full_name=None):
        ...
        AdminMainWindow.resize(1000, 600)
        ...
        self.users_btn = self.create_button(self.nav_panel, "Пользователи", 70)
        self.users_btn.clicked.connect(self.open_user_window)
        self.roles_btn = self.create_button(self.nav_panel, "Роли", 125)
        ...
        self.welcome_label.setText(f"Добро пожаловать, {full_name} (Администратор)")

        self.create_dashboard_card("Всего пользователей", "12", 60, "#caffbf")
        self.create_dashboard_card("HR-ролей", "5", 300, "#bdb2ff")
        self.create_dashboard_card("Попыток входа", "3", 540, "#ffadad")
...
if __name__ == "__main__":
    ...
    ui.setupUi(window, full_name="Сидоров А.А.")
    window.show()
    sys.exit(app.exec())
