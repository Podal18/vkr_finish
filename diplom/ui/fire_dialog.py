...
class FireDialog(QtWidgets.QMainWindow):
    ...
    def load_lateness_report(self):
        ...
        try:
            with connection.cursor() as cursor:
                cursor.execute("""
                    SELECT e.id AS employee_id, e.full_name, COUNT(*) AS late_count
                    FROM employees e
                    JOIN attendance a ON e.id = a.employee_id
                    WHERE a.status = 'late' AND e.is_active = 1
    ...
    def display_report(self, data, report_type):
        try:
            ...
            table.setColumnCount(3)  # Добавлен столбец для ID
            table.setHorizontalHeaderLabels(["ID", "Сотрудник", "Кол-во опозданий"])
            ...
            for row_idx, row in enumerate(data):
                table.setItem(row_idx, 0, QtWidgets.QTableWidgetItem(str(row['employee_id'])))
                table.setItem(row_idx, 1, QtWidgets.QTableWidgetItem(str(row['full_name'])))
                table.setItem(row_idx, 2, QtWidgets.QTableWidgetItem(str(row['late_count'])))
    ...
    def show_lateness_table_for_employee(self):
        ...
        for i, row in enumerate(data):
            ...
            table.setItem(i, 1, QtWidgets.QTableWidgetItem(row["notes"] or ""))
